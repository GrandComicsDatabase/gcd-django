...
from apps.oi.models import *  # noqa
...
def get_preview_generic_image_tag(revision, alt_text):
    ...
    if revision.image_file:
        ...
        return mark_safe(
            '<img src="' + revision.scaled_image.url +
            '" alt="' + esc(alt_text) +
            '" class="' + img_class +
            '" width="' + str(width) + '"/>')
...
def get_preview_image_tag(revision, alt_text, zoom_level, request=None):
    if revision is None:
        return mark_safe(
            '<img class="no_cover" src="' + settings.MEDIA_URL +
    ...
    if revision.changeset.state == states.APPROVED:
        ...
        if revision == current_cover:
            ...
            if (request and request.user.has_perm('gcd.can_approve') and
                    not settings.BETA):
                ...
                # for old covers do manual scaling, since it is
                # the uploaded file
        ...
        img_url = (NEW_COVERS_LOCATION +
                   revision.changeset.created.strftime('%B_%Y/').lower() +
                   suffix)
    ...
    for width in [100, 200, 400]:
        ...
        if cover.is_wraparound and width == 400:
            ...
            width = (float(full_cover.size[0]) /
                     float(cover.front_right - cover.front_left) * 400.0)
            size = (int(width),
                    int(width / full_cover.size[0] * full_cover.size[1]))

            if size[1] < 400:
                size = (int(full_cover.size[0] * 400.0 / full_cover.size[1]),
                        400)
...
def process_edited_gatefold_cover(request):
    ...
    if cd['cover_id']:
        ...
        if CoverRevision.objects.filter(cover=cover,
            ...
            return render_error(
                request,
                (
                    'There currently is a <a href="%s">pending '
                    'replacement</a> for this cover of %s.'
    ...
    try:
        # essentially only needed at beginning of the month
...
def handle_gatefold_cover(request, cover, issue, form):
    ...
    return render_to_response(
        'oi/edit/upload_gatefold_cover.html',
        {
...
def handle_uploaded_cover(request, cover, issue, variant=False):
    ...
    try:
        ...
        # sometimes uploads misbehave. connection dropped ?
    ...
    try:
        ...
        changeset.delete()
        os.remove(destination.name)
        info_text = 'Error: File "' + scan.name + '" is not a valid picture.'
        return _display_cover_upload_form(request, form, cover, issue,
...
def finish_cover_revision(request, revision, cd):
    ...
    return HttpResponseRedirect(urlresolvers.reverse(
        'upload_cover_complete', kwargs={'revision_id': revision.id}))
...
def upload_variant(request, issue_id):
    ...
    # request is a GET for the form
        ...
        kwargs = {'pending_variant_adds': Changeset.objects.filter(
            issuerevisions__variant_of=issue,
...
def upload_cover(request, cover_id=None, issue_id=None):
    ...
    # check if there is a pending change for the cover
    if cover_id and CoverRevision.objects.filter(
            cover=cover, changeset__state__in=states.ACTIVE):
    ...
    if cover:
        ...
        if issue.has_covers():
            kwargs['current_covers'] = get_image_tags_per_issue(
                issue, "current covers",
                ZOOM_MEDIUM, as_list=True, variants=True)
...
def handle_uploaded_image(request, display_obj, model_name, image_type,
    ...
    if not form.is_valid():
        return _display_image_upload_form(
            request, form, display_obj, model_name, image_type)
...
def upload_image(request, model_name, id, image_type, image=None):
    ...
    if image:
        if image.object != display_obj:
            return render_error(request, 'Image and object id do not match.',
                                redirect=False, is_safe=True)
        kwargs = {
...
def general_guidelines(type_name):
    return [
        'The uploaded scan needs to be a readable scan of the %s.' % type_name,
        ('Please use large images scanned with at least 150 DPI, '
         'where 300 DPI is preferred.'),
        ('Upload only the %s and not a scan of the full page, '
         'i.e. without any artwork.') % type_name,
    ]
...
def _display_image_upload_form(request, form, display_obj, model_name,
    ...
    if image_type == 'BrandScan':
        kwargs['header'] = ('Upload a %s scan of the brand emblem' %
                            kwargs['upload_type'])
        kwargs['guidelines'] = [
            'Please only upload an image of the brand emblem.']
